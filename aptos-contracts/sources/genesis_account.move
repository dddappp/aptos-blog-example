...
module aptos_blog_example::genesis_account {
    use aptos_blog_example::resource_account;
    ...
    public fun assert_genesis_account(account: &signer) {
        assert!(signer::address_of(account) == @aptos_blog_example, error::invalid_argument(ENOT_GENESIS_ACCOUNT));
    }

    public(friend) fun resource_account_signer(): signer {
        resource_account::resource_account_signer(@aptos_blog_example)
    }

    public fun resouce_account_address(): address {
        let res_account = resource_account::resource_account_signer(@aptos_blog_example);
